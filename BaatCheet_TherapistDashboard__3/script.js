...
function acceptReq(id){
  ...
  // 1. Logic to match Razorpay.js Tiers
  let amt = 0;
  if (S.role === 'therapist') {
    amt = 2000; // Clinical Psychologist rate
  } else {
    // Check priority from the request name/issue
    amt = r.issue.includes('YELLOW') ? 250 : 150; // Trained vs Guided Listener
  }

  const dur = 45; // Fixed duration for demo consistency
  const sesId='ses_'+Date.now();
  
  S.sessions.unshift({
    id:sesId, name:r.name, issue:r.issue,
    time:new Date().toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit'}),
    date:'Today', dur:`${dur} min`, amt:`Rs.${amt}`, status:'done', init:r.init
  });

  // 2. Update Financial State
  ...
  showT('Session Accepted', `Earned Rs.${amt}`, 'ic-teal');
}
...
async function startTherapistVerification() {
    ...
    S.profile = { name, email, rci: rciNum, role: 'therapist' };
...
function startDashboardListener() {
  const email = S.profile.email;
  const role = S.role; // 'therapist' or 'listener'

  if (!email || !role) return;

  // This listener only shows requests meant for the user's specific role
  db.collection('triage_logs')
    .where('triage.targetRole', '==', role)
    .onSnapshot(snapshot => {
      // Convert the database docs into the local S.requests array
      S.requests = snapshot.docs.map(doc => {
        const data = doc.data();
        return {
          id: doc.id,
          name: "User " + doc.id.slice(-4), // Simple masked name
          issue: "Priority: " + data.triage.listenerTier.toUpperCase(),
          time: 'Live',
          init: 'U'
        };
      });
